...
def get_schema(pytree):
    return jax.tree_util.tree_map(lambda x: type(x), pytree)
...
def restore_schema(pytree, schema):
    ...
    def restore_schema_map(tensor, type):
        ...
        return tensor
    return jax.tree_util.tree_map(restore_schema_map, pytree, schema)
...
def batch_dim_slice(batch, slice_idx):
    return jax.tree_util.tree_map(lambda t: t[slice_idx, ...], batch)
...
def split(pytree, size, axis, result_backend=None):
    ...
    split_pytree = jax.tree_util.tree_map(lambda t: split_op(t, size, axis=axis), pytree)
    split_pytree = squeeze(split_pytree, axis=axis, result_backend=result_backend)
    return jax.tree_util.tree_transpose(
        inner_treedef=jax.tree_util.tree_structure([_ for _ in range(size)]),
        outer_treedef=jax.tree_util.tree_structure(pytree),
...
def to_numpy(pytree):
    return jax.tree_util.tree_map(lambda t: np.asarray(t), pytree)


def to_jax(pytree):
    return jax.tree_util.tree_map(lambda t: jnp.asarray(t), pytree)


def is_numpy(pytree):
    return all(isinstance(l, np.ndarray) for l in jax.tree_util.tree_leaves(pytree))


def is_jax(pytree):
    return all(isinstance(l, jnp.ndarray) for l in jax.tree_util.tree_leaves(pytree))
